...
- 2026-09-26 | Qoder | **推送清零 + core 抽取开工登记（用户拍板 core 优先）**。①39 个本地提交（轮57~96，HUD/机位/侧条打磨）已推 origin/main（12e6d7f..3b45ab2），本地=远端。②微信前置 game core 抽取动工：抽 `src/game/core.ts`（纯 Three.js 运行时，零 React 零 DOM，node 端可 import 自验），分三刀：刀1 渲染骨架（场景/赛道/角色/自动巡航 createGame(canvas)→start/stop）、刀2 输入总线（window 键盘触控→input 抽象，wx.onTouch 同接口）、刀3 音频+结算+遥测回调；每刀三门禁（tsc/build/verify-app 11项+perf≥50），任一刀可停。③共享文件：App.tsx/PerformanceMonitor.tsx 在抽壳后退化为薄壳属可见行为重构，按本条预登记，改法=仅拆调用不改交互语义，影响=HUD 数据流由 props 直读改 core onEvent 回调。④step-5 的 `wechat-export.mjs --core` 壳已 spike 验证（2.69MB<4MB），core 就位后直接出包，微信真机预览需人工（开发者工具在人手）。⑤恢复环境：本轮对话用户把 Qoder 从 opencode 迁入 Qoder CN（内置浏览器），门禁自验走 `APP_URL=http://localhost:3001`。- 2026-09-26 | Qoder | **刀1/刀2/刀3 一次落地（6af35ac，分支 ai/qoder/core-extract）**。①`src/game/core.ts` 2060 行：零 React/零 JSX，唯一 DOM 触碰=`make2D` 工厂（可注入，node import 安全）；入口 `createGame({parent|canvas,width,height,palette,settings,create2DCanvas,devicePixelRatio})` → `start/stop/dispose/resize/reset/setSettings/setCameraView/togglePaused/onEvent/input/debug`。②`GameCanvas.tsx` 从 2316 行退化为 407 行薄壳：HUD 数据流改 core `onEvent` 帧末 flush 去重（state/planks/score/multiplier/progress/paused/cameraView/perf/finished），confetti 从 core 移壳（监听 finished 沿），`sound`/`metricsStore` 留 core（两者本就 DOM-free）。③输入总线已抽象：window 键盘/pointer 监听写 `handle.input{left,right,dragging}`，小游戏壳 `wx.onTouch` 同字段直写即可；键盘 R/1-6/Space 也走 core 内 reset/togglePaused，壳按钮与秒传条同步改调 handle 方法。④行为零回归证据：门禁 tsc 0 错 / build 17s / verify-app 11/11（进度 48→105m、缺口铺板 141m 未溺水、FPS60、镜头 x3+暂停可交互、音效默认关）；输入探针：拖右 steerOffset-1.13→playerX+8.89（同 r76 手感口径）、KeyD/KeyA ±14 饱和、Space 暂停 true→false、秒传 3 落到 Z117 bridging、0 pageerror。⑤@step-5：`wechat-export.mjs --core` 可对接——入口传 `{canvas: wx.createCanvas(), width, height, devicePixelRatio, palette, settings}`，其余与 web 完全一致；core 在 `ai/qoder/core-extract` 分支，合并回 main 前不动 GameCanvas/characterBuilder 属地。⑥下一步：分支合回 main（等 main 侧无在途 GameCanvas 改动即合），随后微信导出 dry-run 验包体。
