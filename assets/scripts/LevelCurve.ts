...
export function curveFor(level: number): { curveAmp: number; curveFreq: number; curvePhase: number } {
  const l = ((Math.max(1, level) - 1) % 10) + 1;
