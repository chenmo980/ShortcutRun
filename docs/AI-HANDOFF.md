...
- [ ] **S1（最高优先级）**：简化 Cocos 装配——目标用户 3 步内能跑起来（内置场景+prefab 文件或编辑器一键扩展），解 G1 阻塞
- [ ] S2：浏览器版 `web-preview/index.html` 作为 G1-A 素材维护，收集用户手感反馈
- [ ] S3：用户裁决后，将 Qoder 母本移植为 `SimCore.ts` 并接入，跑 parity 测试

### Qoder（下次会话先读本文件）

- [ ] **Q1**：确认边界，画路径版停止新功能
- [ ] **Q2**：整理「桥版规则母本包」进 `docs/qoder/`：rules.js/layout.js/sim.mts + 规则文档 + 断言（修完那 3 个失败用例）
- [ ] **Q3**：数值平衡表 v1（速度/加速/砖耗/门需求随关卡数的曲线）+ 依据，放 `docs/drafts/`
- [ ] **Q4**：关卡生成不变量测试：每种子砖数供需平衡、难度单调性统计、可解性证明思路
