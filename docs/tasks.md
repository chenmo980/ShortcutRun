...
- [ ] **v0.2 进化（2026-09-22 启动，目标两周成“能看的 Demo”）**
- [x] V1 角色：程序化骨骼小人（Mixamo 打不开，改零素材方案：头/身/四肢正弦摆动，双版本已上线并验证）
- [x] V2 主题配色：Theme.ts 主题系统（城市默认+糖果），双版本按 T 换肤，verify-web 11/11 含换肤断言
- [ ] V3 UI 流程：开始界面 / 结算面板 / 关卡进度条（HUD 已有关卡号+最佳成绩文本）
  - [ ] V4 音效：吃砖 / 铺桥 / 胜利 / 失败（浏览器版可代码合成）
  - [ ] V5 微信小游戏构建 + 真机预览
- [x] L1-L10 难度曲线接入（LevelCurve.ts + GameApp 关卡循环，2026-09-21）
- [ ] G1 手感微调：基于“有角色有关卡进阶”的版本复审手感

## 时间预期（说实话）

| 里程碑 | 时间 | 说明 |
|---|---|---|
| 能看的 Demo（角色+主题+UI+音效） | **2 周** | 我全年代码，你配合 Mixamo 点击 |
| 能真机玩的微信版 | +3 天 | 构建+适配 |
| 能上架卖广告的商品 | **1-3 个月** | 大头是版号等待（已在 compliance.md 排期），叠加留存调优 |
