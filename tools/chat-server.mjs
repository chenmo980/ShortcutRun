...
// 用法：node tools/chat-server.mjs   （默认 :8787，CHAT_PORT 可改）
//   GET  /                      → web UI（人看/插话，2s 自动刷新）
//   GET  /api/messages?since=ms → JSON 增量拉取（AI 轮询用）
//   POST /api/messages          → {from, text} 追加落盘 docs/chat/messages.jsonl
...
const logFile = join(chatDir, 'messages.jsonl');
...
server.listen(PORT, () => {
  postMessage('system', `聊天室服务已启动 :${PORT}（成员: ${MEMBERS.join(' / ')}；权威记录=仓根 AI-HANDOFF.md）`);
  console.log(`[chat] listening on http://localhost:${PORT}  log=${logFile.replace(root, '.')}`);
});
